...
class SingleTripletDataset(Dataset):
    ...
    def __getitem__(self, index):
        a_idx, p_idx, n_idx, nn_idx = self.triplets[index]
        anchor, positive, negative, nnegative = self.data[a_idx], self.data[p_idx], self.data[n_idx], self.data[nn_idx]
        if self.transform is not None:
            anchor = self.transform(anchor)
            positive = self.transform(positive)
            negative = self.transform(negative)
            nnegative = self.transform(nnegative)
        return anchor, positive, negative, nnegative

    def __len__(self):
        return len(self.triplets)

class SingleTripletDatasetOne(Dataset):
    def __init__(self, anom_idx, x, y, triplets_selector, transform=None):
        self.transform = transform
        self.data = x
        self.triplets = triplets_selector.get_triplets(anom_idx, x, y)
...
class SingleTripletDatasetTwo(Dataset):
    def __init__(self, anom_idx, x, y, triplets_selector, transform=None):
        self.transform = transform
        self.data = x
        self.triplets = triplets_selector.get_triplets(anom_idx, x, y)
    ...
    def __getitem__(self, index):
        n_idx, nn_idx, a_idx = self.triplets[index]
        negative, nnegative, anchor = self.data[n_idx], self.data[nn_idx], self.data[a_idx]
        if self.transform is not None:
            negative = self.transform(negative)
            nnegative = self.transform(nnegative)
            anchor = self.transform(anchor)
        return negative, nnegative, anchor
...
class MyHardSingleTripletSelector:
    def __init__(self, nbrs_num, rand_num, nrand_num, nbr_indices, onbr_indices):
        self.x = None
        self.y = None
        ...
        self.onbr_indices = onbr_indices

    def get_triplets(self, anom_idx, x, y, normal_label=0, abnormal_label=1):
        ...
        anom_idx_nw = np.where(self.y == abnormal_label)[0]
        nbr_indices = self.nbr_indices
        onbr_indices = self.onbr_indices
        rand_num = self.rand_num
        nrand_num = self.nrand_num


        #找到两个数组中集合的差异，即找不含有nbr_indices的noml_idx
        rand_canddt = np.setdiff1d(noml_idx, nbr_indices)
        #print('rand_canddt', rand_canddt)
        #nrand_canddt = np.setdiff1d(anom_idx_nw, onbr_indices)
        rand_indices = np.random.choice(rand_canddt, rand_num, replace=True)
        #nrand_indices = np.random.choice(nrand_canddt, nrand_num, replace=False)
        #print("rand_indices", rand_indices)
        #print("onbr_indices", onbr_indices)

        triplets = [[anchor, positive, anom_idx, nanom]
                    for anchor in rand_indices
                    for positive in nbr_indices
                    for nanom in onbr_indices]
        return torch.LongTensor(np.array(triplets))

class MyHardSingleTripletSelectorOne:
    def __init__(self, rand_num, nbr_indices):
        self.x = None
        self.y = None
        self.rand_num = rand_num
        #self.nrand_num = nrand_num
...
class MyHardSingleTripletSelectorTwo:
    def __init__(self, rand_num, nbr_indices, onbr_indices):
        self.x = None
        self.y = None
        self.rand_num = rand_num
        self.nbr_indices = nbr_indices
    ...
    def get_triplets(self, anom_idx, x, y, normal_label=0):
        ...
        triplets = [[anom_idx, nanom, anchor]
                    for nanom in onbr_indices
                    for anchor in rand_indices]
        #print("***************MyHardSingleTripletSelectorTwo",len(torch.LongTensor(np.array(triplets))))
        return torch.LongTensor(np.array(triplets))
